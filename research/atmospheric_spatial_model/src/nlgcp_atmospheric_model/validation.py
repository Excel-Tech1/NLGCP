...
def loocv_folds(
    ...
    for held in stations:
        ...
        folds.append({
            ...
            "reason": None if len(refs) >= min_refs else (
                f"only {len(refs)} references remain < minimum {min_refs}"
            ),
        })
    return folds
...
def run_loocv(
    *,
        ...
        for epoch in epochs:
            for sat in satellites:
                ...
                if key not in observed:
                    ...
                    continue
                ref_values = [
                    (r, coords[r], observed.get((epoch, sat, r))) for r in refs
                ]
                for model in models:
                    pred = interpolate(
                        model_name=model,
    ...
    for model in models:
        ...
        observed_list = [observed[key] for key in sorted(common_keys)]
        summary[model] = {
